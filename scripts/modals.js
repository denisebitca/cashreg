/* MicroModal v0.4.2 Copyright (c) 2017 Indrashish Ghosh, under MIT License */
...
MicroModal.init({awaitOpenAnimation: true, awaitCloseAnimation: true});

function createModal(title, contentelm, footerelm, options){
    if($("#modal-1").length){
        MicroModal.close("modal-1")
        $("#modal-1").remove()
    }
    contentelm = $($.parseHTML(contentelm))
    let modal = $($.parseHTML('<div class="modal micromodal-slide" id="modal-1" aria-hidden="true"></div>'))
    let two = $($.parseHTML('<div class="modal__overlay" tabindex="-1" data-micromodal-close></div>'))
    let three = $($.parseHTML('<div class="modal__container" role="dialog" aria-modal="true" aria-labelledby="modal-1-title"></div>'))
    let four = $($.parseHTML('<header class="modal__header"></header>'))
    let five = $($.parseHTML('<h2 class="modal__title" id="modal-1-title">' + title + '</h2>'))
    let six = $($.parseHTML('<button class="modal__close" aria-label="Fermer" data-micromodal-close></button>'))
    let seven = $($.parseHTML('<main class="modal__content" id="modal-1-content">' + (()=>{
        var l = "";
        for(var i = 0; i < contentelm.length; i++){
            l += contentelm[i].outerHTML;
        } 
        return l;
    })() + '</main>'))
    let eight = $($.parseHTML('<footer class="modal__footer"></footer>'))
    two.append((()=>{
        four.append(five);
        four.append(six)
        three.append(four);
    ...
    $("body").prepend(modal)
    if(footerelm){
        footerelm.forEach(elm =>{
            $(".modal__footer").append($("<button></button>").addClass("modal__btn").attr('id', elm.id).text(elm.text).css({'background-color': elm.bgcolor, 'color': elm.color}).one("click", elm.callback))
        })
    }
    $(".modal__footer").append($("<button></button>").addClass("modal__btn").attr({"data-micromodal-close":"", "aria-label":"Fermer"}).text("Fermer"))
    try{if(options){
        if(Object.prototype.hasOwnProperty.call(options, "largemodal")){
            if(options.largemodal){
                $(".modal__container").css("max-width", "initial")
            }
        }
    }}catch(e){}
